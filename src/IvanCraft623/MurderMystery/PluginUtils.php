...
class PluginUtils {
	...
	public static function setZip(string $arena) {
		...
		if ($level !== null) {
			$level->save(true);
			$levelPath = Murder::getInstance()->getServer()->getDataPath() . 'worlds' . DIRECTORY_SEPARATOR . $arena;
			...
			foreach ($files as $file) {
				if ($file->isFile()) {
					...
					$localPath = substr($filePath, strlen(Murder::getInstance()->getServer()->getDataPath() . 'worlds'));
